...
Deno.serve(async (request) => {
  ...
  if (!url || !anon || !serviceKey) return json(503, { error: "server_not_configured" });
  ...
  if (body.action === "deletion_preview") return json(200, { active_subscription_platforms: platforms });
  ...
  if (body.action === "export") {
    ...
    for (const table of tables) {
      ...
      if (error) return json(500, { error: "export_failed", table });
      records[table] = data;
    }
    return json(200, { format: "comboreel-account-export", version: 1, exported_at: new Date().toISOString(), account: { id: user.id, email: user.email, created_at: user.created_at }, records });
  }

  if (body.action !== "delete") return json(400, { error: "invalid_action" });
  const token = authorization.replace(/^Bearer\s+/i, "");
  const encodedPayload = token.split(".")[1].replace(/-/g, "+").replace(/_/g, "/");
  const payload = JSON.parse(atob(encodedPayload.padEnd(Math.ceil(encodedPayload.length / 4) * 4, "=")));
  if (!payload.iat || Date.now() / 1000 - Number(payload.iat) > 600) return json(401, { error: "recent_sign_in_required" });
  if (platforms.length && body.acknowledge_active_subscriptions !== true) return json(409, { error: "active_subscription_acknowledgement_required", active_subscription_platforms: platforms });
  ...
  if (stripeCustomer?.customer_id && Deno.env.get("STRIPE_SECRET_KEY")) {
    ...
    if (!response.ok) return json(502, { error: "stripe_account_cleanup_failed" });
  }
