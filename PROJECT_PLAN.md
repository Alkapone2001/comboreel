...
- Hosted staging now has all twelve reviewed migrations applied with matching local/remote history. Edge Functions use Supabase's new publishable/secret key dictionaries, reject legacy JWT-based API-key dependencies, and explicitly authenticate viewers or provider callbacks with gateway JWT verification disabled as required by the new key model.
